...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not curr_place.is_hive:
            if curr_place.bees and self.min_range <= x <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown == 0 and self.place.bees:
            choose_b = choose_bee(self.place.bees)
            Bee.reduce_health(choose_b, choose_b.health)
